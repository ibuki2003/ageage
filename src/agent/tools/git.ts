...
export async function git_commit_call(args: string): Promise<string> {
   ...
   * On generation failure or empty result, the error is thrown and commit is aborted.
   */
  const { message: origMessage } = JSON.parse(args);
  let message = origMessage;
  if (typeof message !== "string" || message.trim() === "") {
    // Auto-generate commit message using OpenAI; abort on failure or empty message
    ...
    try {
      ...
      generated = get_output_text(completion)?.trim()
        || (completion.choices?.[0]?.message?.content ?? '').trim();
      if (!generated) {
        throw new Error("Empty commit message generated by OpenAI");
      }
      message = generated;
      console.log(`Auto-generated commit message: ${message}`);
    } catch (e) {
      console.error(`Failed to auto-generate commit message: ${e}`);
      throw e;
    }
  }
